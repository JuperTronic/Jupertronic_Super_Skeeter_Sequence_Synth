/*   Skeeter Sequence Synth - Arpeggio Box
      ...
      MIDI output to send sequences to external synth
      Sync output for Korg Volca, Pocket Operator, etc
      Drone Glitch latching button to hold root note for jamming
      Outputs to sencond Arduino for OLED display
...
const int NOTES[numSequences][numNotes] = {
  ...
  {0, 12, 12, 0, 12, 12, 0, 12},                    // do do da do da
...
void updateControl() {
  ...
  if (outMIDItoggle == LOW) {
    if (currentRoot + oscOne_offset != lastRoot) {   //Don't change MIDI out note unless the root note changes
      //      MIDI.sendNoteOff(lastRoot - 12, 127, 1);               //This will allow drone or pad instead of pulsing bass notes
      //      MIDI.sendNoteOn(currentRoot - 12, 127, 1);
      MIDI.sendNoteOff(lastRoot - 12 + oscOne_offset, 127, 1);       //enable if you want the pitch pot to affect the note out
      MIDI.sendNoteOn(currentRoot - 12 + oscOne_offset, 127, 1);
  ...
  if (kGainChangeDelay.ready()) {
    ...
    if (droneButtonState == LOW) {
      ...
      if (gain == 0) {                                                    // we'll make changes to the oscillator freq when the note is off
        ...
        //Use toggle switch to determine what to send for MIDI output
        if (outMIDItoggle == HIGH) {
          MIDI.sendNoteOn(NOTES[sequence][note] + currentRoot + oscOne_offset, 127, 1);   // send current note to MIDI out for external synth, full velocity, MIDI channel 1
        }
        note += 1;
        if (note == 0) {
